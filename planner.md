...
- **Known exception (not yet fixed):** `ui/common/render-buckets.js`
  imports `sleep` from `bal/common/compute-stats.js` purely for a
  chat-bubble loading-delay animation. This is a UI→BAL boundary
  violation — `sleep` is a generic timing helper, not business logic,
  and shouldn't live in `bal/`. Left as-is pending a decision on where
  shared non-DOM utils like `sleep` belong (candidate: `core/`).
