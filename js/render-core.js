...
function buildStudentDetailHtml(st){
  ...
  return `<h3 style="font-family:var(--font-display);font-size:18px;margin-bottom:4px">${esc(st.name)}</h3><div style="font-size:12px;color:var(--c-text3);margin-bottom:16px">${idLine}</div>${buildStudentKpiStripHtml(st)}<div class="grid-4" style="margin-bottom:14px">
...
</div>${buildStudentScoreTableHtml(st)}
...
export { _bucketCharts, _charts, _modalFocusTrap, _modalLastFocus, bucketHeatmapHtml, buildSparkPath, buildStudentDetailHtml, buildStudentHealthScoreHtml, buildStudentKpiBottomGridHtml, buildStudentKpiStripHtml, buildStudentKpiTopGridHtml, buildStudentScoreTableHtml, closeModal, configureChartDefaults, dbTabKeyNav, destroyBucketCharts, destroyCharts, downloadUpdatedSheet, filterStudents, getFilteredStudents, getScopedStudents, narrativeCard, openStudentModal, populateIndividualSwitcher, remarkCardsHtml, remarkToneBadgeHtml, renderBucketClassCharts, renderBucketStudentTrendChart, renderBucketSubjectDistChart, renderCharts, renderClassInsights, renderDashboard, renderFlagsTable, renderHeatmap, renderKPIs, renderStudentCards, renderSubjectCorrelation, renderWellbeingPanel, runSampleFile, saveNarrativeField, saveRemarkField, selectIndividualStudent, setFilter, showRemarksDirtyBanner, showSampleFiles, sortStudents, updateExportGate, updateRemarkCharCount };
