...
import { BUCKET_HELP_FLAG_TYPES, BUCKET_TOP_FLAG_TYPES, bucketIsHelp, bucketIsTop } from './render-buckets.js';
...
// NB-7 (prompt.md §11.2): breadcrumbHtml() removed — zero callers since
// the bucket screen flow was replaced by the Dashboard right-rail
// (PIB §6d). The new screen flow renders its own back navigation in
// each right-panel template.
...
// NB-7 (prompt.md §11.2): toggleHelpRow() removed — zero callers since
// the bucket screen flow was replaced by the Dashboard right-rail
// (PIB §6d). The expand-state variable _helpOpenId is also no longer
// needed.
...
export { CHAPTER_RELEVANT_FLAG_TYPES, bucketFindingReason, collectFindingReasons, filterPickerList, flagChapterSuffix, onBucketStudentPick, onBucketSubjectPick, openClusterGroup, openFinding, renderClassAnswer, renderClusterGroups, renderFilteredList, renderStudentPicker, renderSubjectPicker };
