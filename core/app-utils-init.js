...
if("serviceWorker" in navigator&&location.protocol!=="file:"){
  navigator.serviceWorker.register("sw.js").then(function(reg){
    // NB-9 (prompt.md §11.2): controllerchange listener moved INSIDE this
    // .then() so it can safely reference showUpdateBanner (which is
    // defined here). Previously the listener was registered synchronously
    // before this .then() ran, so on the very first install (which fires
    // controllerchange before this callback executes), the `typeof
    // showUpdateBanner === "function"` check evaluated false and the
    // banner silently didn't render. Harmless on first install (banner
    // shouldn't fire then), but defensive-correct: keep both the listener
    // and the function in the same closure scope.
