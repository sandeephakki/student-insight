...
async function onCountryChange(countryCode){
  // Sample-names disclosure moved out of here (BUG FIX): showing it on
  // every plain country switch fired for regular use too — most people
  // changing the Country dropdown never touch Sample Files at all, so
  // the popup was noise for them. It now only fires from
  // render-core.js's runSampleFile(), the moment someone actually tries
  // a sample file while a non-India country is selected — see the
  // gate + comment there. Re-selecting India (countryCode===DEFAULT_COUNTRY)
  // clears any earlier acknowledgment so the notice fires again if they
  // switch away a second time.
  if(countryCode===DEFAULT_COUNTRY) APP._sampleNamesAckCountry=null;
